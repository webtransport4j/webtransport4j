...
/** Test cases for connection migration integration. */
public class ConnectionMigrationIntegrationTest {
  private static final Logger log =
      LoggerFactory.getLogger(ConnectionMigrationIntegrationTest.class);
  ...
  static class UdpProxy {
                ...
                while (running && currentSocket == serverFacing) {
                  try {
                    ...
                    if (lastKnownClientAddr != null) {
                      DatagramPacket fwd =
                          new DatagramPacket(buf, p.getLength(), lastKnownClientAddr);
    ...
    public void migrate() throws Exception {
      log.info(
          "🔴 PROXY: Triggering Connection Migration. Dropping socket on port "
              + serverFacing.getLocalPort());
  ...
  public void testConnectionMigration() throws Exception {
            ...
            stream.onData(
                data -> {
                  ...
                  QuicChannel quicChannel =
                      (QuicChannel) ((NettyWebTransportStream) stream).streamChannel().parent();
                  InetSocketAddress remoteAddress =
                      (InetSocketAddress) quicChannel.remoteSocketAddress();
                        ...
                        new ChannelInboundHandlerAdapter() {
                          ...
                          public void channelRead(ChannelHandlerContext ctx, Object msg) {
                            if (msg instanceof ByteBuf) {
                              ...
                              if (resp.contains("PONG 1")) {
                                preMigrationLatch.countDown();
                              }
                              if (resp.contains("PONG 2")) {
                                postMigrationLatch.countDown();
                              }
                              ((ByteBuf) msg).release();
                            }
                          }
    ...
    assertTrue(
        "Failed to receive PONG 1 before migration", preMigrationLatch.await(5, TimeUnit.SECONDS));
    ...
    assertTrue(
        "Failed to receive PONG 2 AFTER migration! Connection Migration broke!",
        postMigrationLatch.await(5, TimeUnit.SECONDS));
    ...
    assertNotEquals(
        "The client's source port MUST change to prove connection migration!",
        preMigrationAddress.get().getPort(),
        postMigrationAddress.get().getPort());

    assertNotNull("Pre-migration ChannelId should not be null", preMigrationChannelId.get());
    assertEquals(
        "The underlying QUIC Connection object MUST remain identical to prove no new handshake"
            + " occurred!",
        preMigrationChannelId.get(),
        postMigrationChannelId.get());

    log.info(
        "✅ VERIFIED: Server detected port change from "
            + preMigrationAddress.get().getPort()
            + " to "
            + postMigrationAddress.get().getPort()
            + " on EXACTLY the same QUIC Connection (ID: "
            + preMigrationChannelId.get()
            + ")");
