...
public class KeepAliveTest {
  ...
  public void testSessionReaping() {
    ...
    when(mockParent.attr(WebTransportAttributeKeys.PEER_SETTINGS_MAX_STREAMS_BIDI))
        .thenReturn(peerLimitAttr);
    when(mockParent.attr(WebTransportAttributeKeys.PEER_SETTINGS_MAX_STREAMS_UNI))
        .thenReturn(peerLimitAttr);
  ...
  public void testHeartbeatStrictDifferentBytes() throws Exception {
    ...
    try {
      ...
      // Write 0x3F (Heartbeat indicator) followed by incorrect ping byte 0x05, then correct ping
      // byte 0x07
      ByteBuf pingPayload = Unpooled.buffer();
      pingPayload.writeByte(0x3F);
