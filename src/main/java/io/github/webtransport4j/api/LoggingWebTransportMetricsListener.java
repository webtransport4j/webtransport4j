...
/**
 * A built-in {@link WebTransportMetricsListener} implementation that aggregates metric events
 * using lock-free atomic counters and periodically logs a telemetry summary at {@code INFO} level.
 *
...
public class LoggingWebTransportMetricsListener implements WebTransportMetricsListener {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LoggingWebTransportMetricsListener.class);
    ...
    @Override
    public void onConnectionMigration(long sessionId, @NonNull String oldAddress, @NonNull String newAddress) {
        connectionMigrations.incrementAndGet();
        logger.info("🔀 [WT Metrics] Connection Migration | Session: {} | {} → {}", sessionId, oldAddress, newAddress);
    }
    ...
    private void report() {
        ...
        long bRecv = bytesReceived.get();
        long bSent = bytesSent.get();
        long migrations = connectionMigrations.get();

        logger.info("📊 [WT Metrics] Active: {} sessions | Opened: {} | Closed: {} | Streams: {} opened / {} closed",
                active, opened, closed, streamsOpened, streamsClosed);
        logger.info("📊 [WT Metrics] Datagrams: {} recv | {} sent | {} discarded | Bytes recv: {} | Bytes sent: {} | Migrations: {}",
                dgRecv, dgSent, dgDiscard, formatBytes(bRecv), formatBytes(bSent), migrations);
    }
    ...
    public String snapshot() {
        return String.format(
            ...
            activeSessions.get(), totalSessionsOpened.get(), totalSessionsClosed.get(),
            totalStreamsOpened.get(), totalStreamsClosed.get(),
            datagramsReceived.get(), datagramsSent.get(), datagramsDiscarded.get(),
            formatBytes(bytesReceived.get()), formatBytes(bytesSent.get()),
