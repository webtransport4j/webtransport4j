...
/**
 * An interface representing a source of binary data that can be read in chunks. Implementations of
 * this interface should provide a way to read bytes from the source and optionally provide the
...
public interface BinarySource extends AutoCloseable {
   ...
   *
   * <p>The implementation should write into {@code dst} starting at its current position and
   * advance the position by the number of bytes read.
   *
   * @param dst destination buffer
   * @return the number of bytes read, or {@code -1} if the end of the source has been reached.
