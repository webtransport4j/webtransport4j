...
public class WebTransportCapsuleHandler extends SimpleChannelInboundHandler<WebTransportCapsule> {
    ...
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, WebTransportCapsule capsule) throws Exception {
        ...
        if (capsule.capsuleType() == 0x2843L) {
            ...
            logger.info("❌ CLOSE_WEBTRANSPORT_SESSION capsule received. Code: " + errorCode + ", Message: '"
                    + errorMessage + "'. Closing session.");
           ...
           * the peer of the cumulative number of streams of a given type it is permitted
           * to open. A WT_MAX_STREAMS capsule with a type of 0x190B4D3F applies to
           * bidirectional streams, and a WT_MAX_STREAMS capsule with a type of 0x190B4D40
           * applies to unidirectional streams.
           * 
           * Note that, because Maximum Streams is a cumulative value representing the
           * total allowed number of streams, including previously closed streams,
           * endpoints repeatedly send new WT_MAX_STREAMS capsules with increasing Maximum
           * Streams values as streams are opened.
           * 
           ...
           * can be opened over the lifetime of the session. This value cannot exceed 260,
           * as it is not possible to encode stream IDs larger than 262-1.
           ...
           * 
           * Initial values for these limits MAY be communicated by sending non-zero
           * values for SETTINGS_WT_INITIAL_MAX_STREAMS_UNI and
                ...
                if (quic != null) {
                    ...
                    if (mgr != null) {
                        ...
                        if (session != null) {
                            ...
                            long currentLimit = isBidi ? session.getPeerSettingsMaxStreamsBidi() 
                            ...
                            if (maxStreams < currentLimit) {
                                ...
                                // WT_FLOW_CONTROL_ERROR error code."
                                logger.warn("Received lower " + (isBidi ? "Bidirectional" : "Unidirectional")
                                        + " max streams value (" + maxStreams + ") than previously set (" 
                                        + currentLimit + "). Closing session with WT_FLOW_CONTROL_ERROR.");
                                ...
                                if (logger.isDebugEnabled()) {
                                    logger.debug("Updated " + (isBidi ? "Bidirectional" : "Unidirectional")
                                            + " max streams to " + maxStreams 
                                            + " based on received WT_MAX_STREAMS capsule");
                                }
                            }
                        } else {
                            if (logger.isDebugEnabled()) {
                                logger.debug("No session found for ID " + capsule.sessionId()
         ...
         * introduced to inform the peer of the maximum amount of data that can be sent
         * on the WebTransport session as a whole.
         * 
         * This limit counts all data that is sent on streams of the corresponding type,
         * excluding the stream header (see Section 4.2 and Section 4.3). For streams
         * that were reset, implementing WT_MAX_DATA requires that the QUIC stack
         * provide the WebTransport implementation with information about the final size
         * of streams (see Section 4.5 of [RFC9000]).
         * 
         * WT_MAX_DATA Capsule {
         * Type (i) = 0x190B4D3D,
         * Length (i),
         ...
         * with a Maximum Data value less than a previously received value, it MUST
         * close the WebTransport session by resetting the connect stream with the
         * WT_FLOW_CONTROL_ERROR error code.
         * 
         * The WT_MAX_DATA capsule defines special intermediary handling, as described
         * in Section 3.2 of [HTTP-DATAGRAM]. Intermediaries MUST consume WT_MAX_DATA
         * capsules for flow control purposes and MUST generate and send appropriate
         * flow control signals for their limits (see Section 5.6.1).
         * 
         * The initial value for this limit MAY be communicated by sending a non-zero
         * value for SETTINGS_WT_INITIAL_MAX_DATA.
         * 
         * 
         */
                    ...
                    if (mgr != null) {
                        ...
                        if (session != null) {
                            ...
                            if (maxData < currentPeerLimit) {
                                logger.info("❌ Received WT_MAX_DATA (" + maxData + ") less than previous limit ("
                                        + currentPeerLimit + "). Closing session.");
                                mgr.closeSessionWithFlowControlError(capsule.sessionId());
                            } else {
                                session.setPeerSettingsMaxData(maxData);
                                logger.info("📈 WT_MAX_DATA received. Updated peer settings max data to " + maxData
                                        + " for session " + capsule.sessionId());
         ...
         * 
         * A sender SHOULD send a WT_DATA_BLOCKED capsule (type=0x190B4D41) when it
         * wishes to send data but is unable to do so due to WebTransport session-level
         * flow control. WT_DATA_BLOCKED capsules can be used as input to tuning of flow
         * control algorithms.
         * 
         ...
         * appropriate flow control signals for their limits (see Section 5.6.1).
         * 
         * 
         */
                ...
                if (quic != null) {
                    ...
                    if (mgr != null) {
                        ...
                        if (session != null && session.isFlowControlEnabled()) {
                            long extendAmount = WebTransportConfig
                                    .getLong("webtransport4j.webtransport.flowcontrol.extend.data", 10000L);
                            if (extendAmount > 0) {
                                long newLimit = session.getSettingsMaxData() + extendAmount;
                                session.setSettingsMaxData(newLimit);
                                logger.info("📈 Received WT_DATA_BLOCKED. Extending local settings max data to "
                                        + newLimit + " (extended by " + extendAmount + ") for session "
                                        + capsule.sessionId());
         ...
         *
         * A sender SHOULD send a WT_STREAMS_BLOCKED capsule when it wishes to open
         * a stream but is unable to do so due to the maximum stream limit set by its
         * peer. A WT_STREAMS_BLOCKED capsule with a type of 0x190B4D43 is used to
         * indicate reaching the bidirectional stream limit, and a WT_STREAMS_BLOCKED
         * capsule with a type of 0x190B4D44 is used to indicate reaching the
                ...
                if (quic != null) {
                    ...
                    if (mgr != null) {
                        ...
                        if (session != null) {
                            ...
                            long activeCount = isBidi
                                    ? session.getActiveClientInitiatedBi().size()
                                    : session.getActiveClientInitiatedUni().size();
                            long initialLimit = isBidi
                                    ? session.getInitialMaxStreamsBidi()
                                    : session.getInitialMaxStreamsUni();
                            ...
                            if (remaining > 0) {
                                ...
                                logger.info("📈 WT_STREAMS_BLOCKED received. Extending "
                                        + (isBidi ? "bidi" : "uni") + " limit to " + newLimit
                                        + " (blocked=" + blockedAtMax + " + remaining=" + remaining + ")");
                                WebTransportUtils.sendMaxStreamsCapsule(
                                        session.getConnectStream(), isBidi, newLimit);
                            } else {
                                logger.info("ℹ️ WT_STREAMS_BLOCKED received but no remaining active slots. "
                                        + "Not extending limit.");
                            }
                        }
                    }
