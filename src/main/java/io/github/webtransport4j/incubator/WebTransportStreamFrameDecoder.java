...
public class WebTransportStreamFrameDecoder extends SimpleChannelInboundHandler<ByteBuf> {
    private static final Logger logger = Logger.getLogger(WebTransportStreamFrameDecoder.class.getName());
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ByteBuf msg) throws Exception {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(String.format("🖼️ Framing Stream Data: Session: %d | Stream: %d | Bidi: %b | Bytes: %d",
                    sessionId, stream.streamId(), bidirectional, msg.readableBytes()));
        }

        WebTransportStreamFrame frame = new WebTransportStreamFrame(
                sessionId,
                stream.streamId(),
                bidirectional,
                msg.retain()
