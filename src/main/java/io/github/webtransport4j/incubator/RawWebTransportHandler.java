...
public class RawWebTransportHandler extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (ctx.channel() instanceof QuicStreamChannel) {
            ...
            if (!protocolHeaderConsumed) {
                io.netty.util.Attribute<Boolean> serverInitiatedAttr = ctx.channel().attr(WebTransportUtils.SERVER_INITIATED_KEY);
            ...
            if (protocolHeaderConsumed) {
                ...
                if (payloadBytes > 0) {
                    io.netty.util.Attribute<Long> sessionIdAttr = ctx.channel().attr(WebTransportUtils.SESSION_ID_KEY);
                        ...
                        if (mgr != null) {
                            ...
                            if (session != null && session.isFlowControlEnabled()) {
                                ...
                                if (currentReceived + payloadBytes > localLimit) {
                                    logger.warn("❌ Flow control: Read blocked. Cumulative received (" + currentReceived + ") + read (" + payloadBytes + ") exceeds local limit (" + localLimit + "). Closing session.");
                                    mgr.closeSessionWithFlowControlError(sessionId);
                                    data.release();
                                    ((QuicStreamChannel) ctx.channel()).shutdown(WebTransportUtils.WT_FLOW_CONTROL_ERROR, ctx.newPromise());
                                    return;
                                }
                                logger.debug(String.format(">>>>>>>>>>>> %d,%d,%d,%d", currentReceived, (long) payloadBytes, session.getCumulativeBytesReceived(), session.incrementCumulativeBytesReceived(payloadBytes)));
                            }
                        }
                    }
                }
    ...
    /**
     * Intercepts outbound write operations on the WebTransport stream.
     * This is invoked whenever the application (e.g. MessageDispatcher or custom controllers)
     * writes data to a WebTransport stream channel. It tracks and enforces WebTransport
     * session-level flow control rules (Section 5.6 of the WebTransport draft-15 spec).
     *
        ...
        if (currentSent + bytesToWrite > peerLimit) {
            ...
            while ((lastLimit = session.getLastSentDataBlockedLimit().get()) < peerLimit) {
                if (session.getLastSentDataBlockedLimit().compareAndSet(lastLimit, peerLimit)) {
                    logger.warn("❌ Flow control: Write blocked. Cumulative sent (" + currentSent + ") + write (" + bytesToWrite + ") exceeds peer limit (" + peerLimit + "). Sending WT_DATA_BLOCKED.");
            ...
            WebTransportSession.PendingWrite pendingWrite = new WebTransportSession.PendingWrite((QuicStreamChannel) ctx.channel(), data, promise);
            session.getPendingWrites().add(pendingWrite);

            // Add close listener to release the buffer and fail promise if the stream is closed before flushing
