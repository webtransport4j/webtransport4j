...
/** Factory for creating business logic executors. */
public final class BusinessExecutorFactory {
  ...
  private static @NonNull ExecutorService createFixedThreadPool() {
    ...
    final int rawQueueCapacity =
        WebTransportConfig.getInt("webtransport4j.business.queue.capacity", Integer.MAX_VALUE);
    final int queueCapacity = rawQueueCapacity <= 0 ? Integer.MAX_VALUE : rawQueueCapacity;
    final String queueType =
        WebTransportConfig.get("webtransport4j.business.queue.type", "RING_BUFFER");
    logger.info(
        "Using fixed thread pool. poolSize={} , queueCapacity={}, queueType={}",
        poolSize,
        queueCapacity,
        queueType);
        ...
        try {
          ...
          if (!RejectedExecutionHandler.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(
                value + " does not implement " + RejectedExecutionHandler.class.getName());
          }
          return (RejectedExecutionHandler) clazz.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
          throw new IllegalArgumentException(
              "Unable to create RejectedExecutionHandler: " + value, e);
        }
    }
  }
}
