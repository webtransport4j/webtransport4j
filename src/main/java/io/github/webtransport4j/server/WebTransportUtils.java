...
/**
 * Utility constants and methods for WebTransport.
 *
...
public class WebTransportUtils {
  ...
  /**
   * Safely retrieves the {@link WebTransportMetricsListener} from any Channel: resolves the parent
   * QuicChannel automatically if the channel is a QuicStreamChannel.
   */
   ...
   */
  public static @NonNull Future<QuicStreamChannel> createUniStream(
      @NonNull QuicStreamChannel connectStreamChannel,
      boolean byPassLimit,
      @NonNull ChannelHandler streamHandler) {
    return wrapFutureStreamChannel(
        QuicStreamType.UNIDIRECTIONAL, streamHandler, connectStreamChannel, byPassLimit);
  }
   ...
   * @return A Future that completes with the QuicStreamChannel
   */
  public static @NonNull Future<QuicStreamChannel> createBiStream(
      @NonNull QuicStreamChannel connectStreamChannel,
      boolean byPassLimit,
      @NonNull ChannelHandler streamHandler) {
    // 1. Request Stream Creation
    return wrapFutureStreamChannel(
        QuicStreamType.BIDIRECTIONAL, streamHandler, connectStreamChannel, byPassLimit);
  }

  private static @NonNull Future<QuicStreamChannel> wrapFutureStreamChannel(
      @NonNull QuicStreamType quicStreamType,
      @NonNull ChannelHandler streamHandler,
      @NonNull QuicStreamChannel connectStreamChannel,
      boolean byPassLimit) {
    ...
    if (!byPassLimit) {
      long current =
          QuicStreamType.BIDIRECTIONAL == quicStreamType
              ...
              : session.getServerInitiatedStreamsUni();
      long max =
          QuicStreamType.BIDIRECTIONAL == quicStreamType
      ...
      if (current >= max) {
        logger.warn(
            "❌ WebTransport stream limit exceeded for session {}: {} >= {}. Stream creation"
                + " failed.",
    ...
    return connectStreamChannel
        .parent()
        .createStream(quicStreamType, streamHandler)
        .addListener(
            (Future<QuicStreamChannel> future) -> {
              ...
              stream
                  ...
                  .set(connectStreamChannel.streamId());
              stream
  ...
  /** Writes a QUIC variable-length integer to the buffer. */
  ...
  /** Writes a WebTransport capsule to the buffer. */
  ...
  /** Reads a QUIC variable-length integer from the buffer. */
  ...
  private static void sendCapsule(
    ...
    try {
      ...
      if (logger.isTraceEnabled()) {
        ...
        logger.trace(
            "📤 Writing DefaultHttp3DataFrame with {} Capsule (Type: 0x{}, Value/Limit: {}, Total"
                + " Bytes: {})",
            capsuleName,
      ...
      if (future != null) {
        future.addListener(
            f -> {
              if (f.isSuccess()) {
                logger.info(
                    "✅ Capsule writeAndFlush SUCCESS (buffered/handed off to QUIC stack) for Type"
                        + " 0x{}",
                    Long.toHexString(capsuleType));
              } else {
                logger.error(
  ...
  public static void sendMaxStreamsCapsule(
      @NonNull QuicStreamChannel connectStream, boolean isBidi, long maxStreams) {
  ...
  public static void sendDataBlockedCapsule(
      @NonNull QuicStreamChannel connectStream, long maxData) {
  ...
  public static boolean isWebTransportApplicationError(long h) {
    return (h >= WT_ERROR_FIRST && h <= WT_ERROR_LAST && (h - 0x21L) % 31L != 0L)
        || // fallback for Netty QUIC limitation
        (h >= 0 && h <= Integer.MAX_VALUE);
  }

  /** Resets a QUIC stream with the given application error code. */
  ...
  /** Formats the readable bytes of a ByteBuf as a hex dump with an ASCII column. */
  ...
  /** Extracts the parent QUIC channel from a channel handler context. */
  ...
  public static void addTrafficShapers(@NonNull QuicStreamChannel stream) {
    ...
    GlobalTrafficShapingHandler connShaper =
        quic.attr(WebTransportAttributeKeys.CONN_TRAFFIC_SHAPER).get();
    if (connShaper != null) {
