...
public class Http3InboundControlStreamHandler extends SimpleChannelInboundHandler<Http3SettingsFrame> {
    ...
    @Override
    protected void channelRead0(@NonNull ChannelHandlerContext ctx, @NonNull Http3SettingsFrame settingsFrame) {
        ...
        if (settings != null) {
            ...
            if (quic != null) {
                quic.attr(WebTransportAttributeKeys.PEER_SETTINGS_MAX_STREAMS_UNI).set(settings.get(0x2b64L));
                quic.attr(WebTransportAttributeKeys.PEER_SETTINGS_MAX_STREAMS_BIDI).set(settings.get(0x2b65L));
