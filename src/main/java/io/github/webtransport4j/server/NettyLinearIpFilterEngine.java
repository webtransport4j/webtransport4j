...
/** IP filter engine using linear subnet matching. */
  ...
  public void addRule(String cidrOrIp, V value) {
    try {
      ...
      if (cidrOrIp.contains("/")) {
        ...
        rule =
            new IpSubnetFilterRule(parts[0], Integer.parseInt(parts[1]), IpFilterRuleType.ACCEPT);
      } else {
        rule =
            new IpSubnetFilterRule(
                cidrOrIp, cidrOrIp.contains(":") ? 128 : 32, IpFilterRuleType.ACCEPT);
      }
