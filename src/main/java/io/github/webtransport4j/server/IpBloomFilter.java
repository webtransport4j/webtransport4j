 ...
 *
 * <p>NOTE: Bloom filters are probabilistic. They can produce false positives (e.g., claiming an IP
 * is in the blocklist when it isn't), but they never produce false negatives (if an IP is in the
 * list, it will definitely be blocked).
 *
 * <p>To mitigate false positives blocking legitimate users, the default false positive rate is
 * tuned to 1 in 1,000,000,000 (0.000000001).
 */
public class IpBloomFilter {
  ...
  /** Creates a Bloom filter for the expected number of elements and false positive rate. */
  ...
  /** Checks whether the IP might be in the filter (may produce false positives). */
