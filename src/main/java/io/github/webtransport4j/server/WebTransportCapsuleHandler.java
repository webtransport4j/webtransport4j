...
public class WebTransportCapsuleHandler extends SimpleChannelInboundHandler<WebTransportCapsule> {
  ...
  protected void channelRead0(
    ...
    if (capsule.capsuleType() == 0x2843L) {
      ...
      logger.info(
          "🚫 CLOSE_WEBTRANSPORT_SESSION capsule received. Code: {}, Message: '{}'. Closing"
              + " session.",
          errorCode,
          errorMessage);
      ...
      ctx.close();
            ...
            */ if (capsule.capsuleType() == 0x190B4D3FL
        || capsule.capsuleType() == 0x190B4D40L) {
          ...
          if (mgr != null) {
            ...
            if (session != null) {
              ...
              if (maxStreams < currentLimit) {
                ...
                logger.warn(
                    "Received lower {} max streams value ({}) than previously set ({}). Closing"
                ...
                if (logger.isDebugEnabled()) {
                  logger.debug(
                      "Updated {} max streams to {} based on received WT_MAX_STREAMS capsule",
                      isBidi ? "Bidirectional" : "Unidirectional",
                      maxStreams);
                }
              }
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                    "No session found for ID {} when processing WT_MAX_STREAMS capsule",
                    capsule.sessionId());
              }
            }
          }
          ...
          if (mgr != null) {
            ...
            if (session != null) {
              ...
              if (!isFirstUpdate && maxData < currentPeerLimit) {
                ...
                logger.info(
                    "📊 WT_MAX_DATA received. Updated peer settings max data to {} for"
                        + " session {}",
                    maxData,
                    capsule.sessionId());
              }
      ...
      if (maxData != -1) {
        ...
        if (quic != null) {
          ...
          if (mgr != null) {
            ...
            */ if (capsule.capsuleType() == 0x190B4D43L
        || capsule.capsuleType() == 0x190B4D44L) {
              ...
              if (cappedRemaining > 0) {
                ...
                logger.info(
                    "ℹ️ WT_STREAMS_BLOCKED received but no remaining active slots available"
                        + " (active={} initial={} max_extension={}). Not extending limit.",
                    activeCount,
                    initialLimit,
                    maxActiveStreams);
              }
